...
fn login_window(
    ...
    font: &Font,
    image: &Image,
    image_mode: BackgroundMode,
) -> Option<Command> {
    let (display_width, display_height) =
        orbclient::get_display_size().expect("orblogin: failed to get display size");
...
fn main() {
    ...
    loop {
        if let Some(mut command) =
            login_window(&launcher_cmd, &launcher_args, &font, &image, image_mode)
        {
            match command.spawn() {
