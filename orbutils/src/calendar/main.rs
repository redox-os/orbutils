...
impl Calendar {
    pub fn new() -> Self {
        ...
        let mut window_builder =
            WindowBuilder::new(Rect::new(-1, -1, window_width, window_height), "Calendar");
        ...
        let grid = Grid::new();
        grid.position(8, 8 + 16 + 8).spacing(8, 8);
        window.add(&grid);
    ...
    pub fn redraw_grid(&mut self) {
        ...
        for (i, day) in day_names.iter().enumerate() {
            let label = Label::new();
            label
                .size(self.cell_width, self.cell_day_name_height)
                .text(*day)
        ...
        for idx in day_offset..6 * 7usize {
            ...
            match d {
                Some(x) => {
                    ...
                    cell.size(self.cell_width, self.cell_height)
                        .with_class("date")
                        .text(text)
